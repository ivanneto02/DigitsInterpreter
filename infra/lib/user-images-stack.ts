...
export class AlphanumericDetectorImagesStack extends cdk.Stack {
        ...
        constructor(scope: Construct, id: string, props: cdk.StackProps) {
                ...
                // Lambda function gets images from the bucket
                const userImagesFetchingRandomImagesLambda = new lambda.Function(
                        this,
                        "AlphanumericDetectorUserImagesFetchingRandomImagesFunction",
                        {
                                code: lambda.Code.fromAsset("lib/functions/userImagesFetchingRandomImagesFunction"),
                                runtime: lambda.Runtime.NODEJS_LATEST,
                                handler: "index.handler",
                        },
                );

                // Lambda function gets all of a user's images (up to 30) from the bucket
                const userImagesFetchingSpecificUserImagesLambda = new lambda.Function(
                        this,
                        "AlphanumericDetectorUserImagesFetchingSpecificUserImagesLambda",
                        {
                                code: lambda.Code.fromAsset("lib/functions/userImagesFetchingSpecificUserImagesFunction"),
                                runtime: lambda.Runtime.NODEJS_LATEST,
                                handler: "index.handler",
                        }
                );

                // API1 - Getting all user images, this will be random images from the bucket. We only want so many at a time, so I'll opt for randomly selecting 30 images from the bucket.
                const userImagesFetchingAPI = new apigateway.RestApi(
                        this,
                        "AlphanumericDetectorUserImagesFetchingAPI",
                        {
                                cloudWatchRole: false, // NOTE: prevent charges, might remove in the future
                        },
                );

                // For random images
                const userImagesFetchingAPIRandomImagesResource = userImagesFetchingAPI.root.addResource("random-images");
                userImagesFetchingAPIRandomImagesResource.addMethod("GET", new apigateway.LambdaIntegration(userImagesFetchingRandomImagesLambda));

                // For specific users
                const userImagesFetchingAPISpecificUserResource = userImagesFetchingAPI.root.addResource("user-images")
                userImagesFetchingAPISpecificUserResource.addMethod("GET", new apigateway.LambdaIntegration(userImagesFetchingSpecificUserImagesLambda));
        };
};
